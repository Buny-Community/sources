...
impl Source for NovelFire {
	...
	fn get_novel_update(
		...
		if needs_details {
			...
			let cover = main_div.attr("data-src").unwrap();
			...
			let description = html.select(".summary .content p").map(|els| {
				els.filter_map(|el| {
...
// 	#[buny_test]
// 	fn test_request() {
// 		let version = NovelFire::get_latest_buny_version();
